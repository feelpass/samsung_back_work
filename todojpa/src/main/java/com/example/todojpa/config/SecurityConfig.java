...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/").permitAll().antMatchers("/addUser").permitAll().antMatchers("/api/**")
				...
				.antMatchers("/admin/api/**").hasRole("ADMIN").anyRequest().authenticated().and().formLogin()
				.loginPage("/loginform.html").passwordParameter("password").usernameParameter("email")
				.loginProcessingUrl("/login").failureUrl("/userregform.html")
				.successHandler(new CustomneAuthenticationSuccessHandler())
				.permitAll().and().logout()
				.logoutUrl("/logout").logoutSuccessUrl("/userregform.html").permitAll().and().exceptionHandling()
